...
def gtts_speak(jarvis_speech):
    ...
    tts.save('/root/jarvis/jarvis_speech.mp3')
    mixer.init()
    mixer.music.load('/root/jarvis/jarvis_speech.mp3')
    mixer.music.play()
...
if __name__ == '__main__':
    ...
    while True:
        if mode == "voice":
            ...
            response = input("Talk to Your Assistant : ")
        if response.lower().replace(" ", "") in terminate:
            break
        jarvis_speech = kernel.respond(response)
        print("Your Assistant: " + jarvis_speech)
        speak(jarvis_speech)
